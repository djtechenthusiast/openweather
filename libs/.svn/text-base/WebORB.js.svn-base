var divElement;
var consoleLog = null; //function( msg ) {	console.log( msg ); }
...
PubSubProxy.prototype.dispatchMessage = function( message )
{
  ...
  if ( this.DSId == undefined && message["headers"] != undefined && message.headers.DSId != undefined ) 
  {      
...
{       
  ...
  if ( consoleLog != undefined )
    consoleLog( "SENDING MESSAGE " + print( msg ));
...
{
  if ( consoleLog != undefined )
    consoleLog("onClosed (sockets): " + print( error ) );
...
//    data = data.substring(1, data.length-1);
  ...
  if ( consoleLog != undefined )  
    consoleLog( "RECEIVED MESSAGE " + print( message ) );  
...
{    
  url = webORB.runUrlSubstitution( url );
  try 
  {        
	...
	{	
		return self.sockOpen.call( self ); 
	};
    this.sock.onerror = function( error, p2 ) 
    { 
      if( consoleLog != null )
    	  consoleLog( "WEBSOCKET ERROR " + print( p2 ) );
    	  
    	return self.sockError.call( self, error, p2 ); 
    };
    this.sock.onclose = function( evt ) 
    {
      if( consoleLog != null )
    	  consoleLog( "SOCKET CLOSED - " + print( evt ) );
    	if ( self != undefined ) return self.sockClosed.call( self ); 
    };
    
    this.sock.onmessage = function( event ) 
    { 
      if( consoleLog != null )
        consoleLog( "RECEIVED MESSAGE " + print( event ) );
    	return self.sockMessage.call( self, event ); 
    };      
  } 
  catch (e) 
  {
...
for (property in obj) 
{
  var value = typeof( obj[ property ] ) === "function" ? "function" : obj[ property ];
  output += property + ': ' + value+'; ';
  
  if( consoleLog != null )
    consoleLog( property + ': ' + value );
  //output += property + "; ";
}
return output ;
}
...
{ 
  ...
  this.javascript_class_alias = "flex.messaging.messages.CommandMessage";
}
...
{  
  V3Message.call(this);
  this.javascript_class_alias = "flex.messaging.messages.AsyncMessage";
  this.body = data; 
}
...
{  
  ...
  {        
    ...
    {              
      ...
      this.proxy = (webORB.isSocketFormatURL(this.connectionURL) && hasWebSocket()) ? new SocketPubSubProxy( webORBObj, this.reconnectUsingRemoting ) 
...
{  
  ...
  subscribe: function ( subtopic, subscribeAsync )
  {
    ...
    if (!this.proxy || ( webORB.isSocketFormatURL(this.connectionURL) && !this.proxy.isWorking() )) {
      if (hasWebSocket() && webORB.isSocketFormatURL(this.connectionURL)) {
        var webORBObj = new webORB(this.connectionURL, this.userid, this.password);
        webORBObj.producerOrConsumer = this;
        this.proxy = new SocketPubSubProxy(webORBObj, this.reconnectUsingRemoting);
      } else {
        this.connectionURL = webORB.getDefaultRemotingEndpoint();
        var webORBObj = new webORB(this.connectionURL, this.userid, this.password);
        webORBObj.producerOrConsumer = this;
        this.proxy = new RemotingPubSubProxy(webORBObj);
      }
...
{
    this.connectionURL = webORB.runUrlSubstitution( connectionURL );
    this.userid = userid;
    this.password = password;
    
    webORB.registerTypeFactory( "RecordSet", createRecordSet );
    webORB.registerTypeFactory( "flex.messaging.io.ArrayCollection", arrayCollectionToArray );
}

webORB.getDefaultEndpoint = function ( port )
{  
  if( webORB.getDefaultMessagingEndpoint() != undefined )
    return webORB.getDefaultMessagingEndpoint();
    
  if ( window.WebSocket || typeof MozWebSocket !== "undefined")
  {
...
webORB.runUrlSubstitution = function( url )
{
  if( url.indexOf( "{server.hostname}" ) > -1 )
  {
    var hostname = window.location.hostname;
    ...
    url = url.replace( "{server.hostname}", hostname );
  }
...
webORB.getDefaultMessagingEndpoint = function()
{
	if( this.defaultMessagingURL != undefined )
		return this.defaultMessagingURL;
	
	return undefined;
}

webORB.setDefaultMessagingURL = function( defaultMessagingURL ) 
{
	this.defaultMessagingURL = defaultMessagingURL;
}
...
webORB.isSocketFormatURL = function( connectionURL )
{
  ...
  return connectionURL.indexOf( "ws://" ) == 0;

  //if(connectionURL.indexOf("?") != -1)
	//  connectionURL = connectionURL.substr(0, connectionURL.indexOf("?"));  
	  
  //return !(connectionURL.match("weborb.aspx$") == "weborb.aspx" ) && !(connectionURL.match("weborb.wo$") == "weborb.wo" );
}
   ...
   parseResponse: function( responseText )
   {
    ...
    {       
        ...
        //else
          ...
          {
            if( e.message.localeCompare("Access is denied.") && e.number == -2147024891 )
              alert( "Unable to send request to the server. Make sure to load the page from the same server where WebORB is running." );
    ...
    mainParse: function( responseDoc )
    {
		...
		if( isIE() )
			responseNode = responseDoc.firstChild.selectSingleNode( "//Response" );
		else
			responseNode = responseDoc.firstChild.getElementsByTagName( "Response" )[0];
		
        if( responseNode != null )
            return this.parseData( responseNode.firstChild, new Array() );

		if( isIE() )
			responseNode = responseDoc.firstChild.selectSingleNode( "Fault" );
		else
        ...
        switch( dataElement.nodeName )
        {
            case "Object":
                ...
                { 
					...
					if( isIE() )
					{
						fieldName = childNodes[ i ].selectSingleNode( "Name" ).firstChild.nodeValue;
						fieldValueNode = childNodes[ i ].selectSingleNode( "Value" ).firstChild;
					}
					else
					{
						fieldName = childNodes[ i ].getElementsByTagName( "Name" )[ 0 ].firstChild.nodeValue;
						fieldValueNode = childNodes[ i ].getElementsByTagName( "Value" )[ 0 ].firstChild;
					}
    ...
    {        
        ...
        if(isOpera() && isCrossDomain(this.connectionURL))
        {
          alert("When using the Opera browser, make sure to load the HTML file from the same webserver where WebORB is running");
          return;
        }
        if( requestXML.xml )        
    ...
    getProcessedId: function( obj, array )
    {
    ...
    serializeObject: function( obj, xmlNode, processedArray  )
    {
                    ...
                    switch( obj.constructor )
                    {
                        case Array:
                            ...
                            for( var i = 0; i < obj.length; i++ )
                            {
                                ...
                                var arrElementXML = this.serializeObject( obj[ i ], xmlNode, processedArray );
                        ...
                        case Object:
                            return this.createObjectElement( obj, undefined, xmlNode, processedArray );
                            break;
                        ...
                        default:
                            ...
                            if ( className )
                                return this.createObjectElement( obj, className, xmlNode, processedArray );
    ...
    createObjectElement: function( obj, objName, xmlNode, processedArray )
    {
        ...
        var id = this.getProcessedId( obj, processedArray );
        if( id == null )
            processedArray.push( { obj: obj, id: processedArray.length + 1 } );
        else
            return this.createElement( "Ref", id, xmlNode );
        ...
        for( var name1 in obj )
        {
            ...
            valueElement.appendChild( this.serializeObject( obj[ name1 ], xmlNode, processedArray ) );
...
Async.prototype.setLastState = function( stateNum )
{
    ...
    if( this.stateNum == 4 )
    {
    	if( this.commObj.status == 404 )
    	   alert( "Server reported 404. The remoting URL you're using is invalid: " + this.webORBObj.connectionURL );
    	else
...
// transforms "flex.messaging.io.ArrayCollection" to simple Array.
function arrayCollectionToArray( arrayCollectionObj, webORBObj )
{
    if( arrayCollectionObj )
        return arrayCollectionObj["source"];
...
function isCrossDomain(endPoint)
{
  return getHostFromUrl(endPoint) != getHostFromUrl(document.URL);
}
...
function getHostFromUrl(url)
{
  ...
  if(url.indexOf("//") != -1)
  {
    url = url.substring(url.indexOf("//")+2, url.length-url.indexOf("//"));
  }
